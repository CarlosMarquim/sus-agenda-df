...
| **How** (Como) | Implementação em C, com código organizado em módulos por domínio (paciente, médico, agendamento, agenda, menu), cada um com arquivo de implementação `.c` e interface `.h`. Entidades se relacionam por índices inteiros. O mesmo core C é compilado para WebAssembly via Emscripten e executado no navegador, com camada de ponte (`bridge_wasm.c`) expondo funções C ao JavaScript via `Module.ccall()`. O JavaScript atua exclusivamente como camada de apresentação. |
